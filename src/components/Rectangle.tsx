...
import { calculateCornerResize, calculateEdgeResize, isCornerHandle, isEdgeHandle, type Bounds } from '../utils/transform';
...
const Rectangle = ({ shape, isSelected, onSelect, onDragStart, onDragEnd, onUpdateShape, onRightClick }: RectangleProps) => {
  ...
  const handleDragEnd = (e: any) => {
    ...
    // Update local bounds and notify parent
  ...
  const handleResizeDragMove = (handleType: HandleType, e: any) => {
    ...
    // Apply delta based on handle type with proper anchor points
    switch (handleType) {
    ...
    // Apply aspect ratio locking if Shift is held for corner handles
